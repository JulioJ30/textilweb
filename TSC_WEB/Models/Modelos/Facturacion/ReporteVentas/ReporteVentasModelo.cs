    ...
    public class ReporteVentasModelo
    {
        public async Task<List<Ventas>> ListarVentas(string series, DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                using (OracleConnection conexion = DBAccess.ObtenerConexion())
                {
                    ...
                    var resultado = await conexion.QueryAsync<Ventas>(sql: "USYSTEX.SPU_GET_FACTURACION_VENTAS",
                                                                          param: parametros,
        ...
        public async Task<List<Series>> ListarSeries()
        {
            try
            {
                using (OracleConnection conexion = DBAccess.ObtenerConexion())
                {
                    ...
                    var resultado = await conexion.QueryAsync<Series>(sql: "USYSTEX.SPU_GET_FACTURACION_VENTAS",
                                                                          param: parametros,
                                                                          commandType: CommandType.StoredProcedure);
        ...
        public List<VentasReporte> ListaReporte(string series, DateTime fechaInicio, DateTime fechaFin)
        {
            try
            {
                using (OracleConnection conexion = DBAccess.ObtenerConexion())
                {
                    ...
                    var resultado = conexion.Query<VentasReporte>(sql: "USYSTEX.SPU_GET_FACTURACION_VENTAS",
                                                                    param: parametros,
                                                                    commandType: CommandType.StoredProcedure);
        ...
        public MemoryStream ReporteExcel(List<VentasReporte> listado)
        {
            ...
            using (var package = new ExcelPackage(stream))
            {
                ...
                workSheet.Cells["H1"].Value = "NRO FACTURA";
                workSheet.Cells["I1"].Value = "SERIE GUIA";
                workSheet.Cells["J1"].Value = "NUM GUIA";
                workSheet.Cells["K1"].Value = "MONEDA";
                workSheet.Cells["L1"].Value = "PRECIO REAL";
                workSheet.Cells["M1"].Value = "CANTIDAD";
                workSheet.Cells["N1"].Value = "IGV";
                workSheet.Cells["O1"].Value = "VALOR REAL";
                ...
                workSheet.Column(7).Width = 15;     // NRO FACTURA";
                workSheet.Column(8).Width = 15;     // SERIE FACTURA";
                workSheet.Column(9).Width = 15;     // SERIE GUIA";
                workSheet.Column(10).Width = 10;    // NUM GUIA";
                workSheet.Column(11).Width = 10;     // MONEDA";
                workSheet.Column(12).Width = 15;     // SUBTOTAL";
                workSheet.Column(13).Width = 15;     // CANTIDAD";
                workSheet.Column(14).Width = 15;     // IGV";
                workSheet.Column(15).Width = 15;     // TOTAL";
                ...
                foreach (var item in listado)
                {
                    ...
                    workSheet.Cells[i, 8].Value = item.NRO_FACTURA;
                    workSheet.Cells[i, 9].Value = item.SERIE_GUIA;
                    workSheet.Cells[i, 10].Value = item.NUM_GUIA;
                    workSheet.Cells[i, 11].Value = item.MONEDA;
                    workSheet.Cells[i, 12].Value = item.PRECIO_REAL;
                    workSheet.Cells[i, 13].Value = item.CANTIDAD_REAL;
                    workSheet.Cells[i, 14].Value = item.IGV;
                    workSheet.Cells[i, 15].Value = item.VALOR_REAL;
